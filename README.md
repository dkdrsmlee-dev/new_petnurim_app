...
- **결제카드 등록 흐름 (`MembershipCardRegisterScreen` + `TossBillingTestWebViewScreen`, USR-PAY-011)**: 약관 동의 "다음"에서 진입하는 결제카드 등록 단계(Figma USR-PAY-011/012). 국내 표준상 카드번호/CVC/비밀번호를 앱이 직접 받지 않고 **PG(토스페이먼츠) 결제창**에서 입력받으므로 Figma의 커스텀 카드 폼은 쓰지 않고, 이 화면에서 **토스 자동결제(빌링) WebView**를 띄운다. 백엔드 PG 연동(상점 clientKey/secretKey, billingKey 발급·저장 API)이 아직 없어, 현재는 **토스 공개 테스트 clientKey**로 v1 SDK `requestBillingAuth('카드')`를 WebView에서 호출해 **실제 토스 카드 등록창(테스트 모드)** 만 미리 띄운다(`webview_flutter`, success/fail URL 도달 감지 — 휴대폰 변경 `KcpCertWebViewScreen`과 동일 패턴). 실제 billingKey 발급·카드 저장·정기결제는 백엔드 준비 시 실 clientKey+서버 발급으로 교체. 완료/중단 다이얼로그는 디자인대로 공용 `EdgeButtonDialog` 재사용 — 등록 완료("결제 카드가 정상적으로 등록되었습니다" 단일 확인) 시 구독 플로우를 걷어내고 마이펫 상세로 복귀(`popUntil` + 혜택 화면 `routeName`), 뒤로가기 시 중단 확인("카드등록을 중단하시겠어요?" 나가기/계속 등록하기).
