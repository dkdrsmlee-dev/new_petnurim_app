...
- **동작 검증 범위**:
  ...
  - **촬영 미션 배지 정밀 정렬 (`CameraMissionGuideScreen`, Figma 484:26017·484:26018)**: 피그마 코드 내보내기가 누락하는 1px 흰색 그라데이션 테두리를 렌더 픽셀에서 역산해 추가하고, 알약 크기를 피그마 명시값으로 고정했다. 피그마 wrapper의 `top`은 "회전 후 bbox 상단"이라 앱의 `Positioned(top:)`("회전 전 상단")과 기준이 달라 두 배지 위치를 정정했다.
  - **촬영 내역 화면 정정 (`ShootingHistoryScreen`/`CameraHistoryCard`, Figma USR-EVT-019 577:12248)**: 손으로 작성해 깨져 보이던 홈 아이콘을 피그마 원본으로 교체하고, 회색 원을 꽉 채우던 코인 아이콘을 디자인 크기로 줄였다. 촬영 완료 후 내역으로 진입할 때 품종·나이·성별이 비던 문제는 백엔드 내역 응답에 그 정보가 없어서이므로, `CameraScreen` 에 펫 표시 정보를 실어 전달하도록 했다.
  - **토스트 위치 정정 (`ToastUtil`, 전 화면 공통)**: 하단 오프셋이 96 고정이라 시스템 내비게이션 인셋을 고려하지 못해 하단 버튼·GNB를 덮었다. `MediaQuery.padding.bottom + 80` 으로 바꿔 하단 고정 UI 위에 뜨도록 했다.
  - **촬영 미션 배지 섀도우 정정 (`CameraMissionGuideScreen`)**: CSS/피그마의 drop shadow 는 박스 안쪽에 그려지지 않는데 Flutter `BoxShadow` 는 박스 아래에 깔린다. 반투명 배지라 그림자가 비쳐 겹이 하나 더 있어 보이던 것을, 배지 모양을 도려낸 바깥 그림자만 그리도록 바꿨다.
  - **촬영 미션 일러스트 크기·정렬 정정 (`CameraMissionGuideScreen`)**: 일러스트 14개 요소가 디자인의 90% 크기로 들어가 있어 원래 크기로 되돌렸고, 좌표 기준을 화면 폭이 아니라 `Padding` 안쪽 Stack 폭으로 바꿔 통째로 16 오른쪽에 치우쳐 있던 것을 중앙에 맞췄다.
