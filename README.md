...
- **동작 검증 범위**:
  ...
  - **하단 GNB 아이콘 피그마 반영 (`CustomGnb`, Figma 339:20717)**: GNB 아이콘이 `assets/icons/{ic_home}_{active|inactive}.png`를 참조했는데 **해당 디렉터리가 존재하지 않아(pubspec 미선언) 항상 `errorBuilder`의 Material 아이콘(20px)으로 그려지던** 문제 수정 — 디자인과 전혀 다른 아이콘이 노출되고 있었다. 피그마 원본 SVG 10종(`Icon/Home·Gift·Mycam·Rank·Event/24` × on/off)을 내려받아 `assets/images/gnb/ic_gnb_{home|gift|mycam|rank|event}_{on|off}.svg`로 추가(피그마가 Home만 on/off를 뒤집어 export → 색상 `on #7F4FFF`/`off #6C737F` 기준으로 교정)하고 `Image.asset`+Material 폴백을 **`SvgPicture.asset(24×24)`**로 교체(`GnbMenu.iconAsset(bool)`), 라벨에 피그마 line-height `1.4` 적용. 아이콘 활성색(`#7F4FFF`)과 라벨 활성색(`#7025FF`)이 디자인상 다른 값이라 SVG 원본 색을 그대로 사용. 실단말(`R3CR209JAWX`) 검증: 홈 활성 상태 5개 아이콘 확대 대조 일치·탭 시 "준비 중" 토스트 동작 유지(홈 외 탭은 선택 전환이 없는 사양이라 나머지 4개 활성 상태는 위젯 렌더로 피그마 5개 variant 전부 대조).
  - **펫 카드 아이콘 피그마 반영 (`NurimPetCard`, Figma 215:6440)**: 마이펫 목록·마이페이지 펫 카드의 아이콘이 SVG가 아니라 **`CustomPainter` 손그림**이라 디자인과 형태가 달랐던 문제 수정. 피그마 원본 SVG로 교체 — 멤버십 `Icon/Crown/20`(별형 크라운+꼭짓점 점 3개, `#87909E`, `ic_crown_20.svg`), 리워드 `Icon/Coin/20`(`ic_coin_20.svg` — 기존 `ic_coin.svg`(마이펫 상세용 보라)와 **동일 지오메트리**라 회색 `#51565F` 20px 버전으로 파생), 대표펫 배지 `Icon/Favorite/24`(20dp 금색 원 `#F0C119`+흰 별 — 기존 `CircleAvatar` 24dp `#F4C21B`+Material `star_rounded`에서 교정, 배경 `rect`가 섞여 export돼 미사용 상태였던 `ic_favorite.svg`를 원본으로 교체), 우측 화살표 `Icon/ArrowRight/24`(`#909AA9`, `ic_arrow_right_24.svg` — 기존 Material `arrow_forward_ios` 16dp에서 교정). 손그림 클래스 3종(`_CrownIcon`/`_CrownPainter`, `_CoinStackIcon`/`_CoinStackPainter`, `_PrimaryPetBadge`) 제거. 실단말(`R3CR209JAWX`) 검증: 마이페이지 펫 카드 캡처를 픽셀 실측해 크라운 16.3×16.7dp·코인 16.0×13.3dp·배지 20.7dp·화살표 8.7×15.7dp와 색상 4종(`#87909E`/`#51565F`/`#F0C119`/`#909AA9`)이 피그마 스펙과 일치함을 확인. 칩(멤버십 가입하기)의 16dp 화살표는 Material `chevron_right`가 디자인(`Icon/ArrowRight/16`)과 형태 동일해 유지.
