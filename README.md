...
- **멤버십 구독 약관 동의 화면 (`MembershipTermsAgreementScreen`, USR-PAY-012)**: 멤버십 혜택 "멤버십 즉시 구독하기"에서 진입하는 구독 약관 동의 단계(Figma 277:14344 미동의 / 523:16973 전체동의). **회원가입 약관 동의(`TermsScreen`, target=SIGNUP)와는 별개** — 로그인 사용자가 `GET /api/v1/terms?target=SUBSCRIPTION`(인증 불필요)에서 약관을 **동적으로 받아** 렌더(별도 `subscriptionTermsProvider`, 약관 조회 메서드·`ActiveTerm` 모델·`TermsDetailScreen`은 공용 재사용). 상태 전환을 디자인대로 반영 — 체크박스는 채운 원+흰 체크(미동의 `#E8EBF1`↔동의 `#7F4FFF`), 라벨 굵기/색(Medium `#87909E`↔SemiBold `#30343C`), "다음" 버튼 활성/비활성(`#7F4FFF`↔`#E8EBF1`). 전체 동의↔개별 동기화, 모든 필수 체크 시 "다음" 활성. "다음"은 결제카드 등록 화면으로 이동. 백엔드에 구독/카드/빌링·구독용 약관 동의 저장 API가 아직 없어 동의는 클라이언트 게이트로만 사용.
- **결제카드 등록 흐름 (`MembershipCardRegisterScreen`, USR-PAY-011)**: 약관 동의 "다음"에서 진입하는 결제카드 등록 단계(Figma USR-PAY-011/012). 국내 표준상 카드번호/CVC/비밀번호를 앱이 직접 받지 않고 **PG(토스페이먼츠) 결제창**에서 입력받으므로 Figma의 커스텀 카드 폼은 쓰지 않고, 이 화면에 **토스 자동결제(빌링) WebView가 들어갈 자리(placeholder)** 로 흐름만 구성. 백엔드 PG 연동(clientKey/redirect URL, billingKey 발급·저장 API) 부재로 실제 결제창은 미연결이며, 완료/중단 다이얼로그는 디자인대로 구현(공용 `EdgeButtonDialog` 재사용) — 등록 완료("결제 카드가 정상적으로 등록되었습니다" 단일 확인) 시 구독 플로우를 걷어내고 마이펫 상세로 복귀(`popUntil` + 혜택 화면 `routeName`), 뒤로가기 시 중단 확인("카드등록을 중단하시겠어요?" 나가기/계속 등록하기). 백엔드 준비 시 이 자리에 휴대폰 변경 `KcpCertWebViewScreen`과 동일한 WebView 콜백 패턴으로 토스 빌링 인증을 연결하면 실동작 완성.
...
- **동작 검증 범위**:
  ...
  - **결제카드 등록 흐름 (`MembershipCardRegisterScreen`, USR-PAY-011)**: 약관 "다음" → 결제카드 등록(PG 자리). 카드 폼은 만들지 않고 토스 PG WebView 자리(placeholder)로 두되 흐름(완료/중단 다이얼로그)은 디자인대로 구현. 실단말(`R3CR209JAWX`)에서 약관→카드 등록 이동, placeholder 안내, 등록 완료 미리보기(임시)→완료 다이얼로그→마이펫 상세 정확 복귀(`popUntil`+`routeName`), 뒤로가기→중단 다이얼로그(나가기→약관 복귀/계속 등록하기)까지 전체 흐름 검증 완료. ※ 실제 결제창은 백엔드 PG 연동(토스 clientKey/redirect URL, billingKey 발급·구독 생성 API) 준비 시 이 자리에 `webview_flutter` 콜백 패턴으로 연결.
