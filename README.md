...
- **동작 검증 범위**:
  ...
  - **멤버십 구독 약관 동의 화면 (`MembershipTermsAgreementScreen`, USR-PAY-012)**: 멤버십 혜택 "멤버십 즉시 구독하기" → 구독 약관 동의(Figma 277:14344/523:16973). 회원가입 약관(target=SIGNUP)과 분리된 별도 화면·`subscriptionTermsProvider`(target=SUBSCRIPTION)로 `GET /api/v1/terms`(public) 동적 조회. 실단말(`R3CR209JAWX`)에서 백엔드 약관 2건(멤버십 구독 이용약관/개인정보 처리방침) 렌더, 미동의↔전체동의 상태 전환(체크 색·라벨 굵기·다음 버튼 활성), 개별 토글↔전체동의 동기화, `>` 약관 상세(`TermsDetailScreen` HTML), "다음" 준비 중 토스트까지 검증 완료. ※ 백엔드 구독/카드/빌링 API 및 구독용 동의 저장 엔드포인트 부재로 동의는 클라이언트 게이트, "다음"은 준비 중 처리.
